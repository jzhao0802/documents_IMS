...
int main (int argc, char* argv[])
{
  ...
  if ((predfl = fopen (predictionsfile, "w")) == NULL)  /*flagged by Jie, this is to output test predicted score vector*/
  ...
  for(i=0;i<testsample.n;i++) {
    ...
    l=loss(testsample.examples[i].y,y,&sparm);   /*flagged by Jie, this is to calculate the loss*/
    avgloss+=l;
    if(l == 0) 
      correct++;
    else
