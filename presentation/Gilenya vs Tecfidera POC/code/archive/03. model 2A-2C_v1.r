...
simulate_data$treatment<- ifelse(simulate_data$treatment==1, -1, 1)
simulate_interact<- apply(simulate_data[, match(total_covar_list , names(simulate_data))] , 2 , function(x){x * simulate_data$treatment})
simulate_data[,match(total_covar_list , names(simulate_data))]<- simulate_interact
#QC
table(raw_data_recode$treatment , simulate_data$treatment)
table(raw_data$pchrlson_2 , simulate_data$treatment)
table(simulate_data$pchrlson_2)
...
#lambda_seq<- c(initial_lambda[-1] , seq(initial_lambda[length(initial_lambda)] , 0 , by=-0.0001))
lambda_seq<- c(initial_lambda[-1] , seq(initial_lambda[length(initial_lambda)] , 0 , length=2000))
...
summary_treatment_effect(observe=test_obs, simulate=test_sim, treatment=test_data$treatment, response=test_data$post_relapse_persist)
#summary_actual_treatment_effect summary_expect_treatment_effect
#                    -0.08545259                     -0.04970122
#                   -0.005935555                       -0.049168

#                    -0.07732075                     -0.09016964
#                    -0.03755017                     -0.08725527
