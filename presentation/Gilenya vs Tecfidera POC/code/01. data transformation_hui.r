...
data_path<- 'D:\\Project_2014\\Data Mining Trainning\\Actual work\\001data'
...
out_path<-'D:\\Project_2014\\Data Mining Trainning\\Actual work\\003output'
setwd(out_path)

raw_data<- read.table('D:\\Project_2014\\Data Mining Trainning\\Actual work\\001data\\pred_analytics_file.csv', header=T, sep=',')
...
#-----------------------------------------------------------------------
#	Keep only matched Gilenya and Tecfidera patients
#	create treatment variable and response variable
#-----------------------------------------------------------------------
# this step may not need, depend on the requirement of project
raw_data<- raw_data[raw_data$match==1,]
dim(raw_data)

# treatment variable: idx_rx=1 means the index medication is Gilenya
raw_data$treatment<- ifelse(raw_data$idx_rx==1, 1, 0)
table(raw_data$idx_rx , raw_data$treatment)
...
idx_spec_01<- ifelse(raw_data$idx_spec_grp==1 , 1 , 0)
idx_spec_02<- ifelse(raw_data$idx_spec_grp==2 , 1 , 0)
idx_spec_03<- ifelse(raw_data$idx_spec_grp==3 , 1 , 0)
idx_spec_04<- ifelse(raw_data$idx_spec_grp==4 , 1 , 0)
idx_spec_05<- ifelse(raw_data$idx_spec_grp==5 , 1 , 0)
sum(idx_spec_01 + idx_spec_02 + idx_spec_03 + idx_spec_04 + idx_spec_05) == length(raw_data$idx_spec_grp)
...
#write.csv(total_covar_list , 'variable attribution1.csv', row.names=F, quote=F)

# codify the attribute of each variable about whether it is summary or detail variable
covar_attr<- read.table('D:\\Project_2014\\Data Mining Trainning\\Actual work\\001data\\variable attribution.csv', header=T, sep=',')
...
# randomly sample 50% of patients as training group, and another 50% as test group
...
table(raw_data$training , raw_data$post_relapse_persist)
...
write.csv(correlation , 'association_with_response.csv', row.names=T, quote=F)
