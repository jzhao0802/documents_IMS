...
data_path<- 'C:\\work\\working materials\\presentation\\Gilenya vs Tecfidera POC\\output'
...
dim(raw_data)
#[3348,289]
raw_data[which(raw_data$treatment==0),setdiff(colnames(raw_data),colnames(interaction_term))]
...
set.seed(100)
index_case<- sample(case , round(length(case)*training_sample_proportion)) #214
set.seed(100)
index_control<- sample(control , round(length(control)*training_sample_proportion))#1460
...
pc <- numeric(2)
OUT<- file('variable_distribution1.txt' , 'w')
for(i in var_list){
  eval(parse(text = paste('x<- raw_data$' , i , sep='')))
...
match(paste(covar_attr$variable, '_interact', sep='') , OR_result$variable)
OR_result$variable
write.csv(re , 'result.csv', row.names=F, quote=F)
paste(covar_attr$variable, '_interact', sep='')
names(covar_attr)
# QC
